function getView(id) {
    $(document).ready( function () {
            $.ajax({
                url: `/availablePhoneNumbers/get/${id}`
            }).then(function (data) {
                console.log(JSON.stringify(data));
                $('.view-0').html(data.id);
                $('.view-1').html(data.number);
                $('.view-2').html(data.telephoneExchange.codeName);
...
function getUpdate(id) {
    $(document).ready( function () {
            $.ajax({
                ...
                $('#update-form').attr('action', `/availablePhoneNumbers/update/${data.id}`)
                $( "input[name='update-0']" ).val(data.id);
                $( "input[name='update-1']" ).val(data.number);
                $( "select[name='update-2']" ).val(data.telephoneExchange.codeName);
...
function getDelete(id) {
    $("#delete-province").attr('href', `/availablePhoneNumbers/delete/${id}`);
